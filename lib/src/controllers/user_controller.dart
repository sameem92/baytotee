...
class UserController extends ControllerMVC {
      ...
      repository.login(user).then((value) {
        if (value != null && value.apiToken != null) {
          ...
          ScaffoldMessenger.of(scaffoldKey?.currentContext).showSnackBar(
              snackBar_baytoty(S.of(state.context).wrong_email_or_password));

        }
      }).catchError((e) {
        loader.remove();
        ScaffoldMessenger.of(scaffoldKey?.currentContext).showSnackBar(
            snackBar_baytoty(S.of(state.context).this_account_not_exist));
      }).whenComplete(() {
  ...
  void register() async {
    ...
    repository.register(user).then((value) {
      ...
      loader.remove();
      ScaffoldMessenger.of(scaffoldKey?.currentContext).showSnackBar(
          snackBar_baytoty(S.of(state.context).this_email_account_exists)
          );
    }).whenComplete(() {
      Helper.hideLoader(loader);
    });
    ...
    if (loginFormKey.currentState.validate()) {
      ...
      repository.resetPassword(user).then((value) {
        if (value != null && value == true) {
          ...
          loader.remove();
          ScaffoldMessenger.of(scaffoldKey?.currentContext).showSnackBar(
              snackBar_baytoty(S.of(state.context).error_verify_email_settings)
          );
